...
/proc/slot2body_zone(slot)
	switch(slot)
		if(ITEM_SLOT_BACK, ITEM_SLOT_OCLOTHING, ITEM_SLOT_ICLOTHING, ITEM_SLOT_BELT, ITEM_SLOT_ID, ITEM_SLOT_ARMOR, ITEM_SLOT_SPECSTORAGE, ITEM_SLOT_UNDERTOP) // OUTERBOUNDS ADDITION - Added armor, secstorage, and undertop
		...
		if(ITEM_SLOT_FEET, ITEM_SLOT_SOCKS) // OUTERBOUNDS EDIT - Adds socks
			return pick(BODY_ZONE_PRECISE_R_FOOT, BODY_ZONE_PRECISE_L_FOOT)

		if(ITEM_SLOT_LEGCUFFED, ITEM_SLOT_UNDERBOTTOM) // OUTERBOUNDS EDIT - Adds underbottom
